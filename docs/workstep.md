...
- [x] Terraform 설치 확인 (`terraform -v`)
- [x] `terraform/` 디렉토리 생성 및 `main.tf` 작성
  - GCS 버킷 리소스 정의
...
- [x] GitHub Repository에 Environment `Production` 생성
- [x] `Production` Environment Secrets에 `GCP_SA_KEY` 등록 (발급한 JSON 키 내용)
...
- [x] 프로젝트 디렉토리 구조 확정 및 엔트리 파일(`index.js`) 생성
...
- [x] Express 라우터 연결 (`/memoApi` 베이스 URI)
- [x] 에러 응답 형식 통일 (`{ "error": "..." }` JSON)
...
- [x] `main` 브랜치 push → GitHub Actions 파이프라인 성공 확인
- [x] 발급된 HTTPS 엔드포인트 주소 확보
- [x] 오피스 내부망 PC에서 전체 CRUD 엔드포인트 호출 테스트
- [x] 외부 IP에서 호출 시 `403` 차단 확인
